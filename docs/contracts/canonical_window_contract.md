...
Defines the canonical temporal window artifact used as PMBRS’s temporal synchronization backbone. This contract aligns to `artifact_base_contract.md` and references `missingness_semantics_contract.md` for missingness semantics.

## Core Rules

- Canonical windows exist on a fixed globally aligned 60-second grid and serve as the shared temporal backbone for all modalities.
- Canonical windows are structural artifacts: they may exist even when all modalities are absent.
...
- Higher-resolution, lower-resolution, event-driven, and multi-scale analyses remain derived views over the canonical timeline rather than independent temporal systems.
